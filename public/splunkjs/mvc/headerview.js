...
function(
    ...
    return BaseView.extend({
        ...
        checkSSO: function(){
            var trustedIP = this.model.webConf && this.model.webConf.entry && this.model.webConf.entry.content && this.model.webConf.entry.content.get('trustedIP');
            this.SSOModeOn = (typeof trustedIP === 'string' && trustedIP.length > 0) && (this.model.config && typeof this.model.config.get('REMOTE-USER') === 'string');
        },
        render: function() {
            this.checkSSO();
        ...
        return BaseView.extend({
            ...
            renderMessages: function(forceUpdate) {
                ...
                if (this.collection.length > 0 || numLegacyMessages > 0) {
                    this.$(".label-warning, .popdown-dialog-footer, .message-list").show();
                    ...
                    this.$(".label-warning, .popdown-dialog-footer, .message-list").hide();
...
function(
    ...
    return Modal.extend({
        ...
        render: function() {
            ...
            this.$(Modal.HEADER_TITLE_SELECTOR).html(_("What's new in 6.1").t());
...
function(
    ...
    var View = BaseView.extend({
        ...
        initialize: function() {
            ...
            if (this.model && this.model.serverInfo && this.model.serverInfo.entry.content.has('isFree')){
                if (splunkUtil.normalizeBoolean(this.model.serverInfo.entry.content.get('isFree'))) {
                    this.isFreeDfd.resolve();
                } else {
                    this.isFreeDfd.reject();
                }
            }else if(this.model && this.model.serverInfo) {
...
function(
    ...
    },
    {
        ...
        createWithBackbone: function(options){
            ...
            options.model.splunkDappNav.once('change', appNavDfd.resolve);
        ...
        _applyAppNavData: function(data, appNav, options, fromCache) {
            ...
            if (console.DEBUG_ENABLED) {
                if (this._lastApply) {
                    console.log('Applying app nav data %o after %o ms', data, new Date().getTime() - this._lastApply);
                } else {
                    this._lastApply = new Date().getTime();
                    console.log('Applying app nav data: %o', data);
                }
            }
            appNav.set({
                nav: data.nav,
        ...
        _cacheValue: function(data, options) {
            if (this._supportsSessionStorage()) {
                ...
                console.log('Caching app nav data: %o', data);
                try {
        ...
        _createFromCache: function(options) {
            if (this._supportsSessionStorage()) {
                try {
                    ...
                    if (cachedData) {
                        console.log('Applying cached app nav data: %o', cachedData);
...
define('splunkjs/mvc/headerview',['require','exports','module','./mvc','./basesplunkview','views/shared/splunkbar/Master','views/shared/appbar/Master','./sharedmodels','helpers/user_agent','models/config'],function (require, exports, module) {
    var mvc = require('./mvc'),
    ...
    var HeaderView = BaseSplunkView.extend({
        ...
        render: function() {
            this.$el.empty().append(this.globalNav.render().el);
            if(this.settings.get('appbar')){
