...
define("leaflet", ["jquery","splunk.util","splunk.config","helpers/user_agent","contrib/text!contrib/leaflet/leaflet.css","contrib/text!contrib/leaflet/leaflet.ie.css"], (function (global) {
    return function () {
        var ret, fn;
       fn = function ($, SplunkUtil, splunkConfig, userAgent, css, iecss) {
...
define('splunk/mapping/LatLon',['require','leaflet','jg_global'],function(require)
{
...
define('splunk/mapping/LatLonBounds',['require','leaflet','jg_global','splunk/mapping/LatLon'],function(require)
{
    ...
    return Base.extend({
        ...
        startListening: function() {
            if (this.model.searchResultsRow)
                this.listenTo(this.model.searchResultsRow, "change destroy error", this.updateData);
            else if (this.model.searchResultsColumn)
                this.listenTo(this.model.searchResultsColumn, "change destroy error", this.updateData);

            if (this.model.state)
                this.listenTo(this.model.state, "change", this._state_change);
        },
...
define('splunkjs/mvc/splunkmapview',['require','exports','module','jquery','underscore','./mvc','./basesplunkview','./messages','views/shared/Map','backbone','./utils','uri/route','util/console','jquery.ui.resizable','splunk.util','./tokenawaremodel','./drilldown','splunk.config'],function(require, exports, module) {
    var $ = require("jquery");
    ...
    var SplunkMapView = BaseSplunkView.extend({
        ...
        normalizeOptions: function(settings, options) {
            settings.set('drilldown', Drilldown.getNormalizedDrilldownType(options.drilldown, { allowBoolean: true }));
        },
        ...
        initialize: function(options) {
            ...
            if (splunkConfig.INDEPENDENT_MODE) {
                if (!this.settings.has('tileSource')) {
                    this.settings.set('tileSource', 'openStreetMap');                    
                }
            }
        ...
        createMap: function() {
            ...
            this._readMapCenterAndZoom();
            this.map.on('boundsChanged', this._onBoundsChanged, this);
            this.map.on('mapClicked', this.emitDrilldownEvent, this);
        ...
        _readMapCenterAndZoom: function() {
            this.model.set({
                currentMapZoom: this.map.getZoom(),
                currentMapCenter: this.map.getCenter().clone()
            });
        },

        emitDrilldownEvent: function(e) {
            if (this.settings.get('drilldown') === 'none') {
                return;
            }
            e = e || {};
            // Normalize data for drilldown event
            var event = _.extend({
            ...
            event.field = data['click.name'] || null;

            _.extend(data, Drilldown.getNormalizedTimerange(this.manager));
            var payload = Drilldown.createEventPayload(event, _.bind(this._doDrilldown, this, e, this.manager));
            this.trigger('drilldown click click:marker', payload, this);
            var drilldownType = Drilldown.getNormalizedDrilldownType(this.settings.get('drilldown'), { allowBoolean: true });
            if (this.settings.get("drilldownRedirect") && drilldownType === 'all' && !payload.defaultPrevented()) {
                payload.drilldown();
            }
        },
        ...
        _doDrilldown: function(event, manager) {
            ...
            if (event.data) {
                if (!_.all(DRILLDOWN_PROPERTIES, function(srcKey) { return !!event.data[srcKey]; })) {
                    // The search results did not contain bound information for the marker. This happens when the
                    // results have not been produced by the geostats command. In this case we use the first 2 fields
                    // (typically latitude and longitude) and send them to the intentions parser
                    var latLonFields = event.fields.slice(0,2);
                    var values = _(latLonFields).map(function(f){ return event.data[f]; });
                    Drilldown.autoDrilldown({
                        name: latLonFields,
                        value: values
                    }, manager, {
                        sync: event.modifierKey,
                        negate: event.altKey
                    });
                } else {
                    var params = _(DRILLDOWN_PROPERTIES).chain().map(function(srcKey, prop) {
                        return [prop, event.data[srcKey]];
                    }).object().value();
                    event.modifierKey = event.ctrlKey;
                    Drilldown.autoDrilldown(event, manager, {
